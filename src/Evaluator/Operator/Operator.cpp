...
namespace Evaluator {
    namespace Operator
    {
        ...
        namespace Literal
        {
            // TODO: Create new use cases for ops
            // think of other ways to manipulate types with symbols, ie + is && or + combines all properties of two Objects similar to polymorphism
            ...
            Result Exponent(const Expr& lhs, const Expr& rhs, Context* context)
            {
                ...
                return Expr(pow(c_lhs.number, c_rhs.number));
            }
            Result Modulus(const Expr& lhs, const Expr& rhs, Context* context)
            {
                Expr c_lhs = Conversion::Convert(lhs, Expr::Type::EX_NUMBER);
                Expr c_rhs = Conversion::Convert(rhs, Expr::Type::EX_NUMBER);
                return Expr((ex_number_t)((int)c_lhs.number % (int)c_rhs.number));
            }
            Result Multiply(const Expr& lhs, const Expr& rhs, Context* context)
            {
                Expr c_lhs = Conversion::Convert(lhs, Expr::Type::EX_NUMBER);
                Expr c_rhs = Conversion::Convert(rhs, Expr::Type::EX_NUMBER);
                return Expr(c_lhs.number * c_rhs.number);
            }
            ...
            Result Add_l_r(const Expr& lhs, const Expr& rhs, Context* context)
            {
                ...
                {
                    return Expr(lhs.boolean || rhs.boolean);
                }
                else
                {
                    // unsuported value
